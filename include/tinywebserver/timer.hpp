...
class Timer {
 ...
 protected:
  /**
   * @brief A struct to compare the Task
   */
  struct PTaskOrder {
   ...
   */
  KVHeap<ID, std::unique_ptr<Task>, PTaskOrder> tasks_;
...
template <typename ID>
bool Timer<ID>::add(ID const &task_id, std::function<void()> &&call_back,
                    duration start_delay, int times, duration interval) {
  ...
  auto p_task = std::make_unique<Task>(std::move(call_back), start_delay, times,
                                       interval);
  if (running_) {
...
template <typename ID>
bool Timer<ID>::update(ID const &task_id,
                       std::function<void(Timer::Task &task)> &&call_back) {
  bool ret = false;
  {
    std::lock_guard lock(tasks_mutex_);
...
template <typename ID>
bool Timer<ID>::cancel(size_t task_id) {
  std::lock_guard lock(tasks_mutex_);
...
template <typename ID>
void Timer<ID>::run_one_task(std::unique_lock<std::mutex> &lock,
